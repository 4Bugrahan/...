...
<template>
    <AppLayout>
        <!-- ═══════════════════════════════════════
             ÜST ÇUBUK — geri dönüş linki
        ═══════════════════════════════════════ -->
        <section class="bg-white py-6 border-b border-gray-100">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <Link href="/projeler" class="flex items-center gap-1.5 w-fit text-[#666] hover:text-[#0E7A8C] text-sm font-semibold transition-colors">
        ...
        <!-- ═══════════════════════════════════════
             PROJE DETAYI — görsel galerisi + bilgi
        ═══════════════════════════════════════ -->
        <section class="py-12 bg-white">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="grid lg:grid-cols-2 gap-12">
                    <!-- Görsel galerisi -->
                    <div>
                        <div class="relative bg-gray-50 rounded-2xl overflow-hidden aspect-[4/3] mb-4 border border-gray-100">
                            <img v-if="images.length > 0" :src="images[activeImage]"
                                :alt="`${localizedProject.title} - ${activeImage + 1}`"
                            ...
                            <div v-else class="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200">
                                ...
                                <p class="text-gray-500 text-sm mt-3">{{ trans('projects.no_image') }}</p>
                            </div>
                        </div>
                    ...
                    <!-- Bilgi -->
                    <div>
                        <span v-if="localizedProject.location" class="inline-flex items-center gap-1.5 w-fit text-xs sm:text-sm font-bold text-white bg-[#0E7A8C] px-4 py-2 rounded-full shadow-sm mb-4">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                            {{ localizedProject.location }}
                        </span>
                        <h1 class="text-2xl sm:text-3xl lg:text-4xl font-black text-[#1B3163] mb-6 leading-tight">{{ localizedProject.title }}</h1>
                        <div v-if="localizedProject.description" class="prose prose-gray max-w-none mb-8">
